...
#include <DRoot.h>
#include <DPubSubTester.h>
#include <ASPubSubTester.h>
#include <LogIt.h>
#include <shutdown.h>
...
void QuasarServer::mainLoop()
{
    ...
    const std::vector<Device::DPubSubTester*>& testers =
        Device::DRoot::getInstance()->pubsubtesters();
    while(ShutDownFlag() == 0)
    {
        ...
        {
            testers[i]->getAddressSpaceLink()->setCounter(counter, OpcUa_Good);
            testers[i]->getAddressSpaceLink()->setTemperature(25.0 + 5.0 * std::sin(counter / 20.0), OpcUa_Good);
        }
    }
